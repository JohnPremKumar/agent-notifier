...
export function createClaudeCodeInstaller(opts: ClaudeCodeInstallerOptions = {}) {
  ...
  return {
    ...
    detect(): Promise<boolean> {
      return Promise.resolve(existsSync(join(home, '.claude')));
    },
    ...
    isWired(): Promise<boolean> {
      ...
      return Promise.resolve(
        EVENTS.every((evt) =>
          (hooks[evt] ?? []).some((h) => h.hooks.some((x) => x.command.includes(MARK))),
        ),
      );
    },
    ...
    uninstall(): Promise<void> {
      ...
      const hooks = (s['hooks'] as Hooks | undefined) ?? {};
      for (const evt of EVENTS) {
        hooks[evt] = (hooks[evt] ?? []).filter(
          (h) => !h.hooks.some((x) => x.command.includes(MARK)),
        );
