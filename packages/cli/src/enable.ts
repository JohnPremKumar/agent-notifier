...
export async function runEnable(opts: EnableOpts): Promise<void> {
  await applyToggle(opts, true);
}
...
export async function applyToggle(opts: EnableOpts, enabled: boolean): Promise<void> {
  ...
  if (opts.tool) {
    ...
    store.update((c) => {
      c.tools[tool] = { enabled };
    });
    console.log(
      `${kleur.green(enabled ? '✓' : '✗')} tool ${tool} ${enabled ? 'enabled' : 'disabled'}`,
    );
    return;
  }
  // project (default)
  ...
  store.update((c) => {
    c.projects[key] = { enabled };
  });
  console.log(
    `${kleur.green(enabled ? '✓' : '✗')} project ${key} ${enabled ? 'enabled' : 'disabled'}`,
  );
}
