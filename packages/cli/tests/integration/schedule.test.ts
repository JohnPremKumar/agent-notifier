...
describe('schedule', () => {
  let home: string;
  beforeEach(() => {
    home = mkdtempSync(join(tmpdir(), 'agentsch-'));
  });
  afterEach(() => rmSync(home, { recursive: true, force: true }));
  const env = () => ({ ...process.env, HOME: home, USERPROFILE: home, APPDATA: home });
  const cfg = (): SchedCfg =>
    JSON.parse(readFileSync(join(home, '.agent-notifier', 'config.json'), 'utf8')) as SchedCfg;
  ...
  it('add auto-generates id when --id omitted', () => {
    execFileSync(
      'node',
      [BIN, 'schedule', 'add', '--allow', '--days', 'mon', '--from', '09:00', '--to', '18:00'],
      { env: env() },
    );
  ...
  it('add rejects when neither --allow nor --deny passed', () => {
    expect(() =>
      execFileSync(
        'node',
        [BIN, 'schedule', 'add', '--days', 'mon', '--from', '09:00', '--to', '18:00'],
        { env: env() },
      ),
    ).toThrow();
  });
